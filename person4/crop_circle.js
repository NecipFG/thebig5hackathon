// crop_circle.js
// Merkez noktadan 60.7km cap ile dairesel kesim, 5mpp -> 15mpp
const { fromFile, writeArrayBuffer } = require('geotiff');
const fs = require('fs');

const INPUT  = 'Site01/ldem_87s_5mpp.tif';
const OUTPUT = 'Site01/ldem_87s_15mpp_circle.tif';

const PPM_IN  = 5;      // kaynak: 5 metre/piksel
const PPM_OUT = 15;     // hedef: 15 metre/piksel
const RATIO   = PPM_OUT / PPM_IN; // 3
const DIAMETER_M = 60700; // 60.7 km
...
async function main() {
    ...
    const arrayBuffer = await writeArrayBuffer([outData], {
        width: outDiam,
        height: outDiam,
        BitsPerSample: [32],
